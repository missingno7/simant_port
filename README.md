...
| `seg7` | SIMTWO | world sim + tile rendering + event loop; `_GetNewMode*`, `_Bounce`, the full `_Get*Dir` family, `_Make{Blk,Red}Queen`/`_Place{Red,Black}Queen`/`_Add{Black,Red}Ants`/`_{Un}RecruitRed`/`_IsValidYard`/`_FindInLionList`/`_SetAntLion`/`_NotMowed`/`_ForceMode{A,B}`/`_MaintainSwarm`/`_SetCasteProd`/`_SetModeProd`/`_GstrB`/`_KillAntLion`/`_FollowCatDir`/`_GrabMap`/`_GetNearbyPatches`/`_StartMigrate`/`_EndMigrate`/`_fracSIN`/`_fracCOS`/`_PlacePillTile`/`_PillGetLife`/`_StorePillarMap`/`_ReplacePillarMap`/`_PillFoodTile`/`_IsPillDead`/`_InitGrassMap`/`_InitSimVars` done | 48 / 282 | mostly rendering |
