...
#: the corpus the PINNED byte-exact gate's graph imports.  ``simant.native.
#: cpuless_skin`` is the same skeleton with the authoritative hand-recovered
#: bodies laid over it as DIRECT CPUless overrides at the same addresses
#: (``implementation = manual_overrides.get(addr, generated[addr])``) — the
#: manual corpus as SKIN.
#:
#: This runner PREFERS the skin: the whole point of a CPUless SimAnt is that
#: what executes is our recovered source wherever we have it.  The skin carries
#: ISLAND-cost overrides, which is admissible HERE and not in the production
#: graph: the gate keys recorded demo input on instruction counts, and an
#: override that costs one dispatch step where the ASM cost N desyncs it.  See
#: ``--corpus``/``--demo`` below — the one place that constraint follows us here
#: is a demo recorded under a different config, which the runner refuses.
SKELETON_CORPUS = "simant.native.cpuless"
SKIN_CORPUS = "simant.native.cpuless_skin"
...
def override_count(package: str) -> int:
    """How many of the corpus's bodies are AUTHORITATIVE hand-recovered
    overrides rather than generated skeleton bodies.  Counted from the emitted
    source itself (overridegen stamps every body it writes), so the banner
    reports what is actually on disk, never what a manifest claims."""
    d = REPO_ROOT / Path(*package.split("."))
    if not d.is_dir():
        return 0
    return sum(1 for p in d.glob("func_*.py")
               if "AUTOGENERATED by scripts/overridegen.py"
               in p.read_text(encoding="utf-8", errors="replace")[:200])


def banner(manifest: dict, plat, key: str, *, exe_present: bool,
           demo: str | None, corpus: str) -> None:
    stats = closure_stats()
    n_over = override_count(corpus)
    print("=" * 72)
    print("SimAnt standalone CPUless runner — the hard wall")
    print(f"Corpus: {corpus_size(corpus)} CPUless module(s) in {corpus}")
    print(f"  skeleton (generated, instruction-exact): "
          f"{corpus_size(corpus) - n_over}")
    print(f"  SKIN (authoritative hand-recovered overrides, "
          f"simant/recovered): {n_over}")
...
def run(key: str, machine, plat, budget: int, corpus: str) -> int:
...
def sweep(boot_dir: Path, game_root: str, corpus: str) -> int:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--corpus", default=None,
                    help="the CPUless body package to execute.  Default: the "
                         f"SKIN corpus ({SKIN_CORPUS} -- the generated "
                         "skeleton with every routable hand-recovered body laid "
                         "over it as a direct override) when it has been built "
                         f"(python scripts/cpuless_promote.py --skin), else the "
                         f"generated skeleton ({SKELETON_CORPUS}).")
    ap.add_argument("--sweep", action="store_true",
                    help="run every promoted body as a root and classify how "
                         "it stops (the host's own evidence; not an oracle)")
    args = ap.parse_args(argv)

    # SKIN preferred, SKELETON as the honest fallback -- and the choice is
    # PRINTED, never silent: which bodies ran is the whole claim of this runner.
    corpus = args.corpus
    if corpus is None:
        corpus = (SKIN_CORPUS if corpus_size(SKIN_CORPUS) else SKELETON_CORPUS)
    if not corpus_size(corpus):
        raise SystemExit(
            f"play_cpuless: no CPUless bodies in {corpus} -- build the corpus: "
            f"python scripts/cpuless_promote.py"
            + (" --skin" if corpus == SKIN_CORPUS else ""))
    ...
    with exe_access_guard_from_manifest(manifest):
        ...
        if args.sweep:
            return sweep(boot_dir, args.game_root, corpus)
