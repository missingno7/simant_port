...
from liftemit import (DEFAULT_EMIT_DIR, DEFAULT_IR,  # noqa: E402
                      boundary_heads_from_ir, load_dosre_tool)
...
def boundary_heads_args(doc: dict) -> list[str]:
    """CLI arguments for dos_re's --boundary-heads, from the IR's own
    ``facts_applied.boundary_heads`` (the same source scripts/liftemit.py
    emits with — the linker's re-emitted callers MUST carry the identical
    observer/resume configuration or the relink would silently drop it).
    The paragraph-key list is materialized as a generated artifact file
    (dos_re's flag takes @FILE)."""
    heads = boundary_heads_from_ir(doc)
    if not heads:
        return []
    path = REPO_ROOT / "artifacts" / "boundary_heads_para.txt"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "# GENERATED by scripts/liftlink.py from recovery_ir.json "
        "facts_applied.boundary_heads (source: simant/facts/"
        "boundary_heads.txt)\n"
        + "".join(f"{cs:04X}:{ip:04X}\n" for cs, ip in sorted(heads)),
        encoding="utf-8")
    return ["--boundary-heads", f"@{path}"]
